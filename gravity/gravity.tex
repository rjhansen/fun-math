...
Rob holds a Master's degree in computer science from the
University of Iowa and will someday (promise, Dad) finally get around
to finishing his Ph.D.  He can be reached at
\href{mailto:rob@hansen.engineering}{\nolinkurl{rob@hansen.engineering}}.

\section*{Why this book?}
From time to time I give detailed explanations of interesting math
problems to teenagers.  I'm now compiling all my notes into one
volume, in the hopes others might find them interesting.

\section*{How to use this book}
Each chapter covers a single math problem that I think is interesting,
and covers it from beginning all the way to a solution.  Some chapters
will use formal proofs, others will rely on casual reasoning.  Each
chapter stands alone and can be enjoyed independent of the others.  If
a chapter doesn't interest you, try another.

Each chapter includes a couple of homework questions for the benefit
of teachers and parents who want to use this in an educational
setting.  The Appendix contains answers to each question.  \textbf{Not
  all questions are appropriate for all readers.}  Some are much
harder than others.  Some expect an answer, and some are meant just to
...
Although reasonable care has been taken to ensure accuracy, the
author is not responsible for any errors.
...
\textit{Don't be scared by this chapter's size!}  Yes, it's very
...
\begin{longtable}{| c | p{.45\textwidth} | p{.45\textwidth} |}
 ...
 \hline
  ...
  \hline
  \rowcolor[gray]{0.95} \rowcolor[gray]{0.95} 9 & \[ 39.86 = 3.986 \times 10^1 \] & When using scientific notation,
  ...
  \hline
  \rowcolor[gray]{0.95} 11 & \[ \frac{3.986 \times 10^{14}}{(6.371 \times 10^6)^2} \] &
  ...
  \hline
  \rowcolor[gray]{0.95} 13 & \[ 6.371 \times 6.371 \approx 40.59 \approx 4.059 \times
  ...
  \hline
  \rowcolor[gray]{0.95} 15 & \[{10^6}^2 = 10^{12}\] & When raising a power to a power,
  ...
  \hline
  \rowcolor[gray]{0.95} 17 & \[ 10^1 \times 10^{12} = 10^{13}\] &
...
the earth's?
\item
In our equations we're careful to use the distance from the earth's
(or the moon's, or Mars's) center.  But if you were to ask a person on
the street how far they are from the earth, they'd say they're
standing on it.  What would happen if they were truly zero distance
away?  (Hint: Newton's theory of gravity stops \mbox{working ---} why?)
\end{enumerate}
...
We've found a gravity cancellation point $3.46 \times 10^8$ meters from the
  ...
  \item
    ...
    L1. There must be something else going on with the earth and the
    moon that's affecting our numbers and throwing them off.  What
    could it be?
  \item
    Do some more reading on Lagrange points.  How is a Lagrange point
    different from a gravity cancellation point?
  \item
  ...
  \item
    ...
    surface.
  \item
    When $D = 0$, the denominator in the equation is also zero.
    Dividing by zero is not allowed in mathematics, so whenever a
    physical theory divides by zero you can be certain either the
    theory stopped being accurate or else the universe just got very
    interesting.  A common joke among physicists is that ``black holes
    are where the Almighty divides by zero''.
  \end{enumerate}
